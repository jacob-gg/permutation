...
#' @param values The vector/data frame column containing the values to be compared
...
#' @examples
#' set.seed(1969)
#' groups <- c(rep('group_a', 500), rep('group_b', 500))
#' values <- c(rnorm(500, 0, 1), rnorm(500, 0.2, 1))
#' dat <- data.frame(groups, values)
#' permutation_t(dat$groups, dat$values)
#'
#' @export
permutation_t <- function(groups, values, iterations = 10^4, seed = 4, one.sided = FALSE){
  ...
  if(length(unique(test_data$groups)) != 2){
